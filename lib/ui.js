...
function ctaBand({ label, title, body, style = "closing-cta", customActions } = {}) {
  return `
<section class="section ${style}">
  ${label ? eyebrow(label) : ""}
  <h2>${title}</h2>
  <p>${body}</p>
  ${customActions ? `<div class="primary-actions" style="justify-content:center">${customActions}</div>` : primaryActions({ style: "centered" })}
</section>`;
}
