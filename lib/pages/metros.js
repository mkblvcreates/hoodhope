...
function categoryLabel(slug) {
  var cat = impactCategories.find((c) => c.slug === slug);
  if (cat) return cat.name;
  var extra = { "food-nutrition": "Food & Nutrition" };
...
function metroResources(metro) {
  ...
  const categoryOptions = [...new Set(metroResourceList.map((r) => r.category))]
    .sort()
    .map((slug) => `<option value="${slug}">${categoryLabel(slug)}</option>`)
    .join("");
  ...
  const cards = metroResourceList
    .map((r, i) => {
      const catName = categoryLabel(r.category);
      return `
...
<section class="section section-tint">
  <div class="resource-toolbar" data-resource-toolbar>
    <div class="resource-toolbar-search">
      <label for="filter-search-${metro.slug}">Search resources</label>
      <input type="search" id="filter-search-${metro.slug}" data-filter-search placeholder="Search by name, service, or address…" autocomplete="off" />
    </div>
    <div>
...
${ui.ctaBand({
  ...
  body: "If you'd like your organization listed as a verified local resource, submit your information for review.",
  customActions: `<a class="button primary" href="/submit-resource/">Submit Your Organization ${icon("arrow", { size: 14 })}</a> <a class="button secondary" href="/partners/">Partner With Us ${icon("arrow", { size: 14 })}</a>`,
})}
`;
}
